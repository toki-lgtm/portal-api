...
3. 保存先フォルダを決める（例: 社内システム内に `02.資格者証` フォルダを新規作成）
4. そのフォルダをブラウザで開き、URL の `…/folders/ここがID` の **ID 部分をコピー**（これが `DRIVE_FOLDER_ID`）
...
## 切替後の挙動
- **新規の資格者証** → 共有ドライブ「社内システム」に保存。DBには `drive:<fileID>` を記録。
